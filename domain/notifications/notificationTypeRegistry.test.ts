...
const CATEGORIES: NotificationCategory[] = ['case', 'task', 'payment', 'scheduling', 'document', 'signature', 'organization', 'system'];
...
describe('NOTIFICATION_TYPES', () => {
  ...
  it("every entry's category is one of the eight defined categories", () => {
    for (const entry of Object.values(NOTIFICATION_TYPES)) {
      expect(CATEGORIES).toContain(entry.category);
    }
  });

  it('covers every one of the eight categories with at least one entry', () => {
