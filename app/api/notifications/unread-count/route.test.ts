...
function idFactory() {
  idCounter += 1;
  return `route-test-unread-${idCounter}`;
}
...
function getRequest(query: Record<string, string>) {
  ...
  return GET(new Request(`http://localhost/api/notifications/unread-count?${params.toString()}`));
}
...
describe('GET /api/notifications/unread-count', () => {
  ...
  it("returns a live count of the caller's own unread in-app deliveries", async () => {
