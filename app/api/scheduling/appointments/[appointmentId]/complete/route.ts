...
import { canEditAppointment } from '@/services/authorizationPolicyService';
import { completeAppointment, SchedulingServiceError } from '@/services/schedulingService';
...
export async function POST(request: Request, { params }: { params: Promise<{ appointmentId: string }> }) {
  ...
  const b = body as { organizationId?: unknown; outcome?: unknown };
  if (typeof b.organizationId !== 'string') {
    return NextResponse.json({ error: 'organizationId is required.' }, { status: 400 });
  }
  if (b.outcome !== 'completed' && b.outcome !== 'no_show') {
  ...
  if (!(await canEditAppointment({ identityId: userId, organizationId, roleKey: role }, dataAdapterMode))) {
    return NextResponse.json({ error: 'Not authorized to complete this appointment.' }, { status: 403 });
  }
