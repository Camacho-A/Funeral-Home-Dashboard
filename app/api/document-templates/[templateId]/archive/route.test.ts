...
function idFactory() {
  idCounter += 1;
  return `document-template-archive-route-test-${idCounter}`;
}
...
function archiveRequest(templateId: string, body: unknown, headers: Record<string, string> = { origin: 'http://localhost', host: 'localhost' }) {
  return POST(new Request(`http://localhost/api/document-templates/${templateId}/archive`, { method: 'POST', headers, body: JSON.stringify(body) }), { params: Promise.resolve({ templateId }) });
}
...
describe('POST /api/document-templates/[templateId]/archive', () => {
  it('rejects a cross-site request (CSRF)', async () => {
    expect((await archiveRequest('t1', {}, { origin: 'http://evil.test', host: 'localhost' })).status).toBe(403);
  });

  it('a role without document.template.manage is refused', async () => {
    const template = await seedTemplate();
    await seedCaller('funeralDirector');
    expect((await archiveRequest(template.id, { organizationId: DEFAULT_ORGANIZATION_ID })).status).toBe(403);
  });

  it('archives the template and records document.template.archived', async () => {
    const template = await seedTemplate();
    await seedCaller('manager');
    const response = await archiveRequest(template.id, { organizationId: DEFAULT_ORGANIZATION_ID });
    expect(response.status).toBe(200);
    expect(documentTemplateFixtures.find((t) => t.id === template.id)?.status).toBe('archived');
  ...
  it('returns 404 for a nonexistent template', async () => {
    await seedCaller('manager');
    expect((await archiveRequest('no-such-template', { organizationId: DEFAULT_ORGANIZATION_ID })).status).toBe(404);
  });
});
