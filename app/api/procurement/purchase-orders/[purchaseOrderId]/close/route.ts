...
import { closePurchaseOrder, PurchaseOrderServiceError } from '@/services/purchaseOrderService';
import { getDataAdapterMode } from '@/lib/env';

/** Phase 36. Close a PO (procurement.manage). Non-posting. */
export async function POST(request: Request, { params }: { params: Promise<{ purchaseOrderId: string }> }) {
  ...
  try {
    const purchaseOrder = await closePurchaseOrder(organizationId, purchaseOrderId, ctx, dataAdapterMode);
