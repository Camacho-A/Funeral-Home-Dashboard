...
import { submitPurchaseOrder, PurchaseOrderServiceError } from '@/services/purchaseOrderService';
import { getDataAdapterMode } from '@/lib/env';

/** Phase 36. Submit a draft PO (procurement.manage). Non-posting. */
export async function POST(request: Request, { params }: { params: Promise<{ purchaseOrderId: string }> }) {
  ...
  try {
    const purchaseOrder = await submitPurchaseOrder(organizationId, purchaseOrderId, ctx, dataAdapterMode);
