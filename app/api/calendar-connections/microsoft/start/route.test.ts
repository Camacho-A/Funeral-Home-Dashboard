...
function startRequest(body: unknown, headers: Record<string, string> = { origin: 'http://localhost', host: 'localhost' }) {
  return POST(new Request('http://localhost/api/calendar-connections/microsoft/start', { method: 'POST', headers, body: JSON.stringify(body) }));
}
...
describe('POST /api/calendar-connections/microsoft/start', () => {
  ...
  it('builds a real Microsoft authorize URL and sets the signed state cookie', async () => {
    const response = await startRequest({ organizationId: DEFAULT_ORGANIZATION_ID });
