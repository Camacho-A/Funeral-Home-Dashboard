...
function idFactory(): string {
  idCounter += 1;
  return `stmt-exclude-route-test-${idCounter}`;
}
...
function postRequest(lineId: string, body: unknown, headers: Record<string, string> = { origin: 'http://localhost', host: 'localhost' }) {
  return POST(new Request(`http://localhost/api/accounting/banking/statement-lines/${lineId}/exclude`, { method: 'POST', headers, body: JSON.stringify(body) }), {
...
beforeEach(async () => {
  ...
  const { lines } = await importBankStatement(
    ...
    { bankAccountId: account.id, fileName: null, statementPeriodStart: null, statementPeriodEnd: null, lines: [{ transactionDate: '2026-08-01T00:00:00.000Z', description: 'Fee', amount: -25 }], createdByStaffProfileId: null, idFactory },
    ctx,
    'mock',
  );
  lineId = lines[0].id;
...
describe('POST /api/accounting/banking/statement-lines/[lineId]/exclude', () => {
  it('rejects a cross-site request (CSRF)', async () => {
    const response = await postRequest(lineId, { organizationId: DEFAULT_ORGANIZATION_ID }, { origin: 'https://evil.example.com' });
    expect(response.status).toBe(403);
  });

  it('returns 403 for a role without accounting.reconcile', async () => {
    mockSession = { user: mockMultiOrgUser };
    const response = await postRequest(lineId, { organizationId: DEFAULT_ORGANIZATION_ID });
    expect(response.status).toBe(403);
  });

  it('excludes the bank-only line', async () => {
    const response = await postRequest(lineId, { organizationId: DEFAULT_ORGANIZATION_ID });
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.line.matchStatus).toBe('excluded');
  });
});
