...
import { createAndPostJournalEntry } from '@/services/generalLedgerService';
import { STARTER_ACCOUNT_NUMBERS } from '@/domain/ledger/starterChartOfAccounts';

let idCounter = 0;
function idFactory(): string {
  idCounter += 1;
  return `je-reverse-route-test-${idCounter}`;
}
...
function postRequest(entryId: string, body: unknown, headers: Record<string, string> = { origin: 'http://localhost', host: 'localhost' }) {
  return POST(new Request(`http://localhost/api/accounting/journal-entries/${entryId}/reverse`, { method: 'POST', headers, body: JSON.stringify(body) }), {
    params: Promise.resolve({ entryId }),
  });
}

let entryId = '';
let lengths: { ledgerAccounts: number; journalEntries: number; journalEntryLines: number; activityEvents: number };
beforeEach(async () => {
...
describe('POST /api/accounting/journal-entries/[entryId]/reverse', () => {
  it('rejects a cross-site request (CSRF)', async () => {
    const response = await postRequest(entryId, { organizationId: DEFAULT_ORGANIZATION_ID, reason: 'Fix' }, { origin: 'https://evil.example.com' });
  ...
  it('returns 403 for a role without accounting.post', async () => {
    ...
    const response = await postRequest(entryId, { organizationId: DEFAULT_ORGANIZATION_ID, reason: 'Fix' });
    expect(response.status).toBe(403);
  });

  it('reverses the posted entry', async () => {
    const response = await postRequest(entryId, { organizationId: DEFAULT_ORGANIZATION_ID, reason: 'Correction' });
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.entry.reversesEntryId).toBe(entryId);
    expect(body.entry.sourceType).toBe('reversal');
  });

  it('rejects reversing the same entry twice', async () => {
    await postRequest(entryId, { organizationId: DEFAULT_ORGANIZATION_ID, reason: 'Correction' });
    const response = await postRequest(entryId, { organizationId: DEFAULT_ORGANIZATION_ID, reason: 'Correction again' });
    expect(response.status).toBe(400);
  });
});
