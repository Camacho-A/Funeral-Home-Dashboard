...
import { getTransactionRegister } from '@/services/financialReportsService';
import { getDataAdapterMode } from '@/lib/env';

/** Phase 31. Transaction Register — gated `accounting.report`. */
export async function GET(request: Request) {
  ...
  const rows = await getTransactionRegister(organizationId, dataAdapterMode, { fromDate, toDate });
