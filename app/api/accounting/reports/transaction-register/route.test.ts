...
function idFactory(): string {
  idCounter += 1;
  return `report-txn-register-route-test-${idCounter}`;
}
...
function getRequest(organizationId: string) {
  return GET(new Request(`http://localhost/api/accounting/reports/transaction-register?organizationId=${organizationId}`));
}
...
describe('GET /api/accounting/reports/transaction-register', () => {
  ...
  it('returns an empty register with no entries', async () => {
