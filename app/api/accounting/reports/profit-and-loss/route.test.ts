...
function idFactory(): string {
  idCounter += 1;
  return `report-pnl-route-test-${idCounter}`;
}
...
function getRequest(organizationId: string) {
  return GET(new Request(`http://localhost/api/accounting/reports/profit-and-loss?organizationId=${organizationId}`));
}
...
describe('GET /api/accounting/reports/profit-and-loss', () => {
  ...
  it('returns zero revenue and expenses with no activity', async () => {
