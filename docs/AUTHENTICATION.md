...
- **API routes do not verify `organizationId` against the caller's session.** `app/api/organizations/[organizationId]/route.ts` (Phase 15A) and `app/api/workflow-templates/*` (Phase 15B) all read `organizationId` from the request and use it directly, without checking it against the authenticated session's actual authorized organization(s) — a gap against this document's own "never trust organizationId supplied by the browser" rule. See `docs/ROADMAP.md`'s "Planned: Multi-Tenant Authorization Hardening" for the full writeup and recommended fix.
