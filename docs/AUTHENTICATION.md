...
|---|---|---|
| `SESSION_JWT_SECRET` | **Private** | HMAC key for Solis's own session cookie — reuses the name reserved in `.env.example` since Phase 0. Falls back to a fixed, clearly-insecure development value outside production (so mock mode needs zero new configuration); **throws in production if unset** |
...
No public registration UI exists, and none was built. Per this phase's own instruction, Solis defaults to invitation-only / administrator-provisioned access. `lib/auth/wixAuth.ts`'s real-mode module does not even expose a `register()` wrapper — only `login()` — so there's no code path that could create a Wix member account, accidental or otherwise, until a future phase deliberately adds one with your review.
...
A third `AUTH_ADAPTER` value, `'identity'`, adds Solis's own email/password identity system alongside the `'mock'`/`'wix'` modes described above (both entirely unchanged by this phase). Highlights:
...
**A genuinely new authorization pattern: the sessionless public signing surface.** Every prior public/token-gated route in this codebase (`verify-email`, `reset-password`, `accept-invitation`) eventually mints a Solis session for the person using it. `/sign` and `/api/signing/*` never do, for anyone, ever — a signer has no Solis account at all. Authorization for these routes reduces entirely to "do you possess a valid, hashed, not-yet-terminal, not-yet-expired token" (the exact same trust model a password-reset link already relies on), with the signer's identity verified out-of-band — the staff member emailed *this specific* signer at *this specific* address — rather than by any Solis credential. `middleware.ts`'s matcher allowlists `/sign` alongside the four existing public identity pages (`/api/*` was already excluded wholesale). No RBAC permission ever gates a public signing route; RBAC governs only the four staff-facing signature routes above.
...
## Phase 32 — Reporting, Analytics & Executive Dashboard

**Status: closed 2026-08-11.** Full writeup: [ADR-036](./adr/ADR-036-reporting-analytics-executive-dashboard-architecture.md); collection details in [WIX_DATA_SCHEMA.md](./WIX_DATA_SCHEMA.md).

Four new RBAC keys — `report.operational`, `report.staff`, `report.export`, `dashboard.manage` — widen the catalog from 50 to **54**. `report.operational`/`report.staff` tier identically to the pre-existing `report.view` (administrator, manager, funeralDirector, readOnly); `report.export`/`dashboard.manage` are administrator/manager only, mirroring `audit.export`'s narrower-than-read precedent. `report.view` (Phase 20) remains the unchanged base "can see Reports/Dashboard at all" gate; financial report access stays exactly as narrow as Phase 31 left it (`accounting.report`) — no duplicate "financial reporting" key was created. `readOnly`'s existing "grants only `*.read`/`*.view` permissions" structural test needed one explicit, documented carve-out for `report.operational`/`report.staff`, which are pure view actions despite not following that suffix naming convention.

`services/authorizationPolicyService.ts` gained `canViewOperationalReports`/`canViewStaffReports`/`canExportReports`/`canManageDashboard`, one-line `hasPermission` wrappers following the exact existing pattern. Every new report/metric route additionally checks `hasPermission(policyParams, mode, definition.permission)` directly against the calling report/metric's own registry-declared permission — a data-driven check parameterized by `domain/reporting/reportRegistry.ts`/`metricRegistry.ts`, not a hardcoded per-route policy function.

## Phase 34 — Scheduling Integrations, Calendar Sync & Automated Reminders

**Status: closed 2026-08-12.** Full writeup: [ADR-038](./adr/ADR-038-scheduling-integrations-calendar-sync-and-reminders.md); collection details in [WIX_DATA_SCHEMA.md](./WIX_DATA_SCHEMA.md).

A genuinely new authorization surface, distinct from every mechanism above: OAuth 2.0 authorization-code flows against two external providers (Google, Microsoft), where the "session" being protected belongs to a third party, not Solis.

- **State parameter, not a session cookie.** `POST /api/calendar-connections/{google,microsoft}/start` is a normal same-origin, cookie-authenticated action (`requireSameOrigin` applies, exactly like every other CSRF-protected mutating route) up to the point it mints a high-entropy random `state` value and redirects the browser to the provider's own authorize URL. The `state` is persisted in a signed, httpOnly cookie (`lib/auth/calendarOAuthState.ts`'s `OAUTH_STATE_COOKIE_NAME`/`oauthStateCookieOptions()`, mirroring `lib/auth/session.ts#cookieOptions()`'s own httpOnly/prod-only-secure/`sameSite=lax` shape) rather than a new Wix collection — a ~5-minute-lived value doesn't warrant persistent storage.
- **The callback route is the one genuinely cross-origin exemption in this system**, alongside the pre-existing Clover webhook (`app/api/webhooks/clover/route.ts`). `GET /api/calendar-connections/{google,microsoft}/callback` never calls `requireSameOrigin` — Google/Microsoft's redirect back to Solis is never same-origin by construction — and substitutes the OAuth `state` check as its authenticity mechanism instead: the callback verifies the returned `state` matches the value in the signed cookie before ever exchanging the authorization code. Same underlying principle as the Clover webhook (skip the origin check, substitute a purpose-built authenticity check), a structurally different mechanism (a server-bound single-use nonce, not an HMAC signature over a request body).
- **Token storage, not session storage.** A successful exchange yields a Google/Microsoft access token and refresh token — these authenticate Solis *to the external provider*, not a user *to Solis*, so they never touch `sessions`/`IdentitySession`. Instead they're AES-256-GCM encrypted (`lib/identity/calendarTokenEncryption.ts`, structurally identical to `lib/identity/mfaSecretEncryption.ts`'s random-12-byte-IV / `iv‖authTag‖ciphertext`-base64 construction) and persisted on the owning `CalendarConnection` row, keyed by a new, dedicated `CALENDAR_TOKEN_ENCRYPTION_KEY` — deliberately a separate key from `MFA_ENCRYPTION_KEY`, since no secret-class key in this codebase is ever shared across unrelated secret classes; compromising one must never expose the other. Same dev-fallback/prod-hard-fail behavior as every other secret-material env var below.
...
Procurement/AP posting-sensitive transitions run under **narrow per-aggregate leases** (`po-{org}-{poId}`, `bill-{org}-{billId}`) reusing the existing lease substrate — no org-wide AP lock — with the same disclosed residual final-fence-to-write race (no Wix OCC/CAS). Vendor payments are **record-only**: Solis posts Dr 2000 AP / Cr Cash but never initiates a bank/ACH/card transfer, and never uses the customer-facing `PaymentRecord`.

## Phase 38 — RBAC Grant Hygiene & Authorization Integrity

**No new permission keys** (catalog stays at **64**). Phase 38 makes the *existing* RBAC model reliable and prevents future grant drift; see docs/adr/ADR-042-rbac-grant-hygiene-and-authorization-integrity.md.

- **Authorization never depends on `createdAt`.** A permission grant is a `rolePermissions` row; authorization is pure permission-key presence (`Set.has`). `mapWixRolePermissionItem` now treats a missing/non-string `createdAt` as **non-fatal** (normalized to a sentinel) so a valid grant is never silently revoked over missing non-security metadata. Security-critical fields stay **fail-closed**: a malformed `id`/`roleId`, or a `permissionKey` not in the current catalog (unknown/stale), still resolves to *no grant*.
- **Grant reconciliation is additive, idempotent, and manual.** `services/rbacReconciliationService.ts` diffs the live **global** platform-default-role grants against `DEFAULT_ROLE_DEFINITIONS` and only inserts missing grants (deterministic ids) or backfills `createdAt` in place — never deletes/overwrites, so it can never strand an org of administrators. `dry_run` is read-only; a second dry-run after `apply` converges to 0 changes. **Nothing runs at startup or on deploy** — production authorization data never mutates merely because the app starts.
- **Scopes stay separate.** Default roles are platform-global (org-null, shared) — reconciliation is a `PLATFORM`-scoped operation and never touches organization-owned custom roles. **Platform administrator** (`PLATFORM_ADMIN_USER_IDS` env allowlist) remains entirely separate from tenant RBAC and is untouched by reconciliation. `organizationId` is always resolved from the authenticated server context, never trusted from the client.
- **Administrator invariant.** Enforced by set-equality (`administrator == PERMISSION_KEYS`), replacing the brittle hardcoded count — a new catalog key not granted to administrator fails CI.
...
## Phase 40 — MFA & Account Security

Activates the Phase-21 TOTP-MFA foundation and adds enforcement (ADR-044). **No new permission keys.**

- **Auth boundary (login challenge):** a correct password on an `mfaEnabled` account no longer creates a session. The login action issues a short-lived (5 min), signed, **non-session** challenge token (`lib/auth/mfaChallengeToken.ts`, `beacon_mfa_challenge` cookie, distinct audience + key context from the staff/family session tokens) and redirects to `/login/mfa`. `submitMfaChallenge` verifies a TOTP code or a single-use recovery code and only then mints the real `IdentitySession`. The challenge token is never accepted as authentication by any protected route.
- **Enrollment / management** (`/api/auth/mfa/{begin,verify,disable,recovery-codes,status}`, self-service, `requireIdentitySession`): two-step enrollment (setup key/otpauth URI → TOTP verify → 10 one-time recovery codes, hashes-only), recovery-code regeneration (TOTP-gated), and disable (TOTP/recovery-gated so a hijacked session can't strip MFA). Secret is AES-256-GCM encrypted at rest (unchanged from Phase 21). No plaintext secret/code/token is ever logged (structural test-enforced). UI: a Security-page MFA panel.
- **Org require-MFA** (`Organization.requireMfa`, additive/nullable/default false): a member of a require-MFA org who isn't enrolled is routed to `/settings/security` to enroll at login — **never hard-locked out**. Set via `PATCH /api/organization/mfa-policy` (`organization.manage`-gated). MFA state and RBAC are kept **separate** — the MFA service/policy import no permission layer.
- **Staff-login rate limiting:** the existing server-authoritative account lockout (`domain/identity/lockoutPolicy.ts`, 5 failures / 15 min, persisted via `loginActivityEvents` + `Identity.status='locked'`) now also counts failed MFA-challenge attempts — brute-forcing the second factor trips the same lockout.
- **Family Portal auth is untouched and isolated** — the `PortalUser`/`familySessionToken` stack has no MFA concept, imports no staff MFA module, and never reads org require-MFA (structural test-enforced).
- **Deferred:** trusted devices (the `rememberDevice` flag only extends TTL, never skips MFA), admin-assisted MFA reset, SMS/email OTP (TOTP-only), WebAuthn/passkeys, SSO/SAML. Continuous mid-session enforcement of a newly-enabled org policy is applied at next login (disclosed).
...
- **Phase 31: `INVOICE_OVERDUE` has no true scheduled trigger.** Solis has no background/cron job infrastructure anywhere in the codebase; this notification type can only be evaluated on-demand (e.g., when a staff member views the AR Aging report), never on a real nightly schedule. Same class of gap as `SchedulingNotifier`'s reserved concrete-delivery mechanism — named, not silently worked around.
- **Phase 32: report/dashboard aggregation is live-computed on every request, no snapshot layer.** Same class of scaling concern as Phase 31's own financial-report note — justified today by Manor's Cremation's genuinely low real volume, not built ahead of need; the named mitigation if volume changes is a period-anchored snapshot table, never a silent degradation of the always-fresh drill-down reports.
- **Phase 32: cycle-time metrics (`cases.average_cycle_days`, `cases.completed`) have real data only in `wix` mode.** `casesService.ts`'s mock-mode branch never calls `activityService.recordCaseCreated`/`recordStageChanged` (only the `wix`-mode API routes do) — a disclosed, pre-existing constraint this phase works around by name rather than by wiring, since doing so would require the mock branch to construct a real `ActivityContext` it structurally has no access to (it executes client-side, in the browser).
- **Phase 34: neither Google's nor Microsoft's real OAuth flow has ever executed against a live provider in this environment.** No real `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`/`MICROSOFT_CLIENT_ID`/`MICROSOFT_CLIENT_SECRET` exist here — the same class of gap `lib/auth/wixAuth.ts`'s real Wix member login has carried since Phase 13. Both `CalendarProvider` adapters are unit-tested against a stubbed `fetch` and structurally complete; the OAuth `state`/callback/token-exchange mechanism itself is also fully unit-tested. Live verification exercised the sync sweep's own Wix-side write path against real production data with `fetch` stubbed only for `googleapis.com` (see ADR-038's "Live Wix verification" section) — it did not and could not exercise a real Google/Microsoft authorization screen or token exchange. Activating either provider in production is an env-var change, no code change, identical in shape to Clover's own sandbox-flag story.
- **Phase 34: `lib/rateLimiter.ts`'s process-local limitation now applies to a third surface** (calendar OAuth `start`/`callback`), alongside staff login (still entirely unmitigated) and the Family Portal (Phase 29, at least in-app-counter-mitigated). Not newly solved by this phase.
