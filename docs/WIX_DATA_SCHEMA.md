...
1. **Wix metadata is kept separate from Beacon domain identifiers.** Every collection has Wix's own system `_id` (opaque, Wix-managed, never referenced by application code) *and* an explicit `beacon<Thing>Id` text field — a Beacon-generated stable string id matching the existing `id` field on the corresponding `types/*.ts` type — this field-naming convention predates the Solis rename (2026-09) and is deliberately kept unchanged to avoid a production data migration for zero user-visible benefit. Every cross-collection reference below is a plain text field holding another collection's `beacon<Thing>Id`, not a formal Wix "Reference" field type (which keys off system `_id`) — so Solis's own code, including `resolveAuthorizationContext`, never has to reason about a Wix-internal identifier.
...
**Purpose:** a real, Solis-owned identity — email/password login, invitations, MFA (Phase 21). Answers "who is this person," never which organizations they belong to or what they can do there. **Ownership:** not organization-scoped — one identity can belong to many organizations via `organizationMemberships`. **Retention:** never hard-deleted; `status: 'deleted'` instead.
...
| `displayName` | Text | Required | Mutable |
| `phone` | Text, nullable | Optional | Mutable — the gate for the SMS notification channel (Phase 33); staff-editable via `PATCH /api/auth/profile` |
...
| `isExternal` | Boolean | Required | Immutable — vendors Solis doesn't operate, never conflict-checked |
...
| `channel` | Text (`in_app`\|`email`\|`sms`) | Required | Immutable — `sms` added Phase 33 |
| `status` | Text (`pending`\|`sent`\|`delivered`\|`read`\|`failed`\|`queued_for_digest`) | Required | Mutable — fully independent of the owning `Notification`'s own `status`; `queued_for_digest` added Phase 33, a holding state only `notificationDigestService.ts`'s sweep ever exits |
...
**Purpose:** one row per `(organizationId, identityId)`; a missing row resolves to both channels enabled by default (a synthetic, unpersisted view — never eagerly seeded). **Ownership:** organization-owned (also identity-scoped). **Retention:** never deleted. Phase 28 shipped it patching only `emailEnabled`/`inAppEnabled`/`updatedAt`; Phase 33 activated every other reserved field (`digestFrequency`/`quietHoursStart`/`quietHoursEnd`/`smsEnabled`) and added `lastDigestSentAt`/`categoryOverrides` — see ADR-032 and [ADR-037](./adr/ADR-037-real-notification-delivery.md).
...
| `digestFrequency` | Text (`instant`\|`daily`\|`weekly`) | Required | Mutable — enforced as of Phase 33 (`domain/notifications/digestTiming.ts`) |
| `quietHoursStart`, `quietHoursEnd` | Text, nullable | Optional | Mutable — enforced as of Phase 33 |
| `smsEnabled` | Boolean | Required | Mutable — the SMS channel shipped in Phase 33, gated by `Identity.phone` |
| `lastDigestSentAt` | Text (ISO timestamp), nullable | Optional | Mutable — added Phase 33; advanced only by `notificationDigestService.ts`'s sweep |
| `categoryOverrides` | Text (JSON-serialized), nullable | Optional | Mutable — added Phase 33; `Partial<Record<NotificationCategory, {emailEnabled, inAppEnabled, smsEnabled}>>`, parsed/stringified only inside `lib/wixNotificationPreferenceMapper.ts`, mirrors `ReportPreset.filters`'s Phase 32 precedent |
...
**Purpose:** the header record for one imported bank statement file (Phase 31) — the least-precedented part of this phase, since no prior Solis feature imports external data. **Ownership:** organization-scoped. **Retention:** never deleted. See ADR-035.
...
**Sales tax — not currently implemented / out of scope.** An initial Phase 37 draft explored configurable sales tax; it was removed before commit per a business-requirements correction. Solis does not calculate, collect, accrue, report, or remit sales tax. The draft had created some live artifacts; the following cleanup was performed (live, `managed-cremations`):
...
## Phase 38 — RBAC Grant Hygiene & Authorization Integrity (Collection 82)

See docs/adr/ADR-042-rbac-grant-hygiene-and-authorization-integrity.md. Phase 38 adds **no new permission keys** and changes **no existing collection's fields**.

### Collection 82 — `rbacReconciliationRecords` (planned — pending the Live Wix Authorization Checkpoint)
One row per reconciliation *run* — the dedicated, rollback-bearing history of grant-hygiene reconciliations, complementary to `organizationRoleAuditEntries` (which remains the human-facing role/membership trail). Backend/API-key access only; deterministic `_id`-as-natural-key (`beaconRbacReconciliationRecordId` = the run id). Fields: `runId` (Text), `mode` (Text enum `dry_run` | `apply`), `scopeMarker` (Text enum `PLATFORM` | `TENANT`), `organizationId` (Text, **null for a PLATFORM/global run** — no per-org fan-out, D5), `actor` (Text — `system:rbac-reconciliation` or an admin identity id), `summaryJson` (Text — JSON-encoded run summary + findings), `changesJson` (Text — JSON-encoded per-change prior state + rollback op), `startedAt`, `completedAt` (Text ISO). Indexes: `(organizationId)` (regular) for tenant-run lookups; a single-field index on `completedAt` if run-history ordering needs it (within the 3-regular cap). Only `services/rbacReconciliationService.ts` writes it.

### `rolePermissions` mapper hardening (no schema change)
`mapWixRolePermissionItem` no longer requires `createdAt` to be a string: a valid grant missing that metadata is normalized to a sentinel rather than dropped (authorization never reads `createdAt`; it is pure permission-key presence). The security-critical fields (`beaconRolePermissionId`, `roleId`, `permissionKey` — catalog-validated) remain fail-closed. The 20 live rows carrying the missing-`createdAt` defect are repaired in place by the reconciliation backfill. **The `rolePermissions` row shape is unchanged** (no `updatedAt` added — D8).
...
No public write access, no unauthenticated read access, no member-self read access. Backend (API-Key-authenticated) access only. Nothing here needs to be broader: Solis's browser code never talks to Wix Data directly — every read/write, once wired in a later phase, goes through Solis's own Next.js server code, which resolves and enforces `organizationId` first. This matches `lib/wixClient.ts`'s existing `ApiKeyStrategy` pattern from Phase 12; no new authorization strategy is needed for these collections.
...
- **Manors Jotform integration, hidden-field identifier correction (2026-09): `externalFormConfigs`' live schema is one field-addition behind the current code.** A live audit of both real Manors Jotforms found that a hidden field's internal `name` is not reliably derived from its requested display name (confirmed empirically: lowercased on some fields, an unrelated auto-generated placeholder like `input274` on others) — `types/externalFormConfig.ts#ExternalFormConfig` was corrected to add `linkTokenFieldQid`/`webhookAuthFieldQid` (both `Text`, matching this collection's established convention), the now-authoritative inbound webhook-parsing identifiers, while `linkTokenFieldName` is retained for its one remaining, narrower role (outbound Jotform URL-prefill only — `domain/externalForms/prefillUrl.ts`). This correction was made LOCAL-CODE-ONLY, deliberately not yet applied to the live `externalFormConfigs` collection or its two already-seeded Manors rows — the two live rows still lack these fields entirely. The exact next infrastructure step: add both fields via `POST /wix-data/v2/collections/create-field` (`Text`, `ADMIN`×4 permissions, matching every other field on this collection), then populate the two existing rows — Vital Statistics (`extform-config-managed-cremations-jotform-vital-statistics`): `linkTokenFieldQid: '44'`, `webhookAuthFieldQid: '45'`; Arrangement Forms (`extform-config-managed-cremations-jotform-arrangement-forms`): `linkTokenFieldQid: '274'`, `webhookAuthFieldQid: '275'` — deferred to a separate, explicitly approved step, not performed as part of this local-only correction.
